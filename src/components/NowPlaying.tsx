...
export default function NowPlaying() {
  ...
  // safe track fields (never crash)
  const track = a.current ?? { title: "—", artist: "Mileage Mafia Radio", src: "" };
  const art = (track as any)?.artwork as string | undefined;
  const title = track.title ?? "—";
  const subtitle = (track as any)?.artist ?? "Mileage Mafia Radio";
    ...
    >
      ...
      >
        ...
        <div className="px-4 pb-4 pt-3">
          ...
          <p className="mt-2 text-[11px] text-neutral-600">Drag the top bar to move.</p>
        </div>
      </div>
    </div>
  );
